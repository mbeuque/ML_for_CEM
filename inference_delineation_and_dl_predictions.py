...
if __name__ == '__main__':
    ...
    model, _ = create_models(backbone_retinanet=backbone.maskrcnn, num_classes=2,
                             weights=args.path_weights, freeze_backbone=False)
    ...
    for i, temp_le_path in tqdm(enumerate(list_paths_low_energy)):
        ...
        mask_prediction, dict_prediction = detect_and_classify_lesions(temp_re_path, temp_le_path,
                                                                       model)
        file = open(args.path_dir_to_save + "/" + ntpath.basename(temp_le_path).split(".")[0], "wb")
